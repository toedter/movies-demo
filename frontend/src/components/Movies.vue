  ...
  <div class="container">
    ...
    <table class="table table-striped table-sm">
      ...
      <tr v-for="movie in movies">
        ...
        <td style="text-align: left;vertical-align:middle;">
          <router-link class="nav-item nav-link" :to="'/movies/' + getMovieId(movie)">{{ movie.title }}</router-link>
        </td>
        <td v-if="!(movie._links.directors instanceof Array)" style="text-align: left;vertical-align:middle;">
          {{ movie._links.directors.name }}
        </td>
        <td v-if="movie._links.directors instanceof Array" style="text-align: left;vertical-align:middle;">
          <div v-for="director in movie._links.directors">{{ director.name }}</div>
        </td>
    ...
    <nav aria-label="Page navigation example">
      <ul class="pagination">
        ...
        <li v-for="index in getMaxPage() - getMinPage()"
            :class="{'page-item':true, 'active':page.number === index-1+getMinPage()}">
          <a class="page-link" href="#"
             v-on:click="getMoviesByPage(index-1+getMinPage())">{{ index + getMinPage() }}</a>
        </li>
...
export default class Movies extends Vue {
  ...
  created() {
    axios.defaults.headers.common['Accept'] = 'application/prs.hal-forms+json';
    this.getMovies();
  }
